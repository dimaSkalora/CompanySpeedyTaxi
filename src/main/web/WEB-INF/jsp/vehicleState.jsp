...
<div class="jumbotron">
    <div class="container">
        <c:url var="createOrUpdateUrl" value="/vehicleStates/createOrUpdate" />
        <form:form method="POST" modelAttribute="vehicleState" action="${createOrUpdateUrl}"
            ...
            <footer>
                ...
                <c:set var="jsf_request_uriStr" value="/CompanySpeedyTaxi/vehicleStates/get"/>
                <c:if test="${jsf_request_uri.startsWith(jsf_request_uriStr)}">
                    <a href="/CompanySpeedyTaxi/vehicleStates" class="btnLogin"><spring:message text="OK"/></a>
                    <%--<input class ="btnLogin" type="button" value="<spring:message text="OK2"/>" onclick="location.href = '/CompanySpeedyTaxi/userStates'"/>--%>
                </c:if>
                <c:if test="${!jsf_request_uri.startsWith(jsf_request_uriStr)}">
                    <c:if test="${!empty vehicleState.id}">
